...
struct BloomTextures {
    #[allow(dead_code)] bloom_a: wgpu::Texture,
    view_a:  wgpu::TextureView,
    #[allow(dead_code)] bloom_b: wgpu::Texture,
...
impl BloomTextures {
    fn new(device: &wgpu::Device, hdr_w: u32, hdr_h: u32) -> Self {
        ...
        Self { bloom_a, view_a, bloom_b, view_b }
    }
}
...
pub struct PostProcessPass {
    ...
    #[allow(dead_code)] swapchain_format: wgpu::TextureFormat,
}
...
impl PostProcessPass {
    pub fn new(
        ...
        let prefilter_pipeline = mk_pipeline(
            "prefilter", &bloom_mod, "vs_fullscreen", "fs_prefilter", &pf_bgl, BLOOM_FORMAT,
        );
        let blur_h_pipeline = mk_pipeline(
            "blur_h", &bloom_mod, "vs_fullscreen", "fs_blur_h", &blur_bgl, BLOOM_FORMAT,
        );
        let blur_v_pipeline = mk_pipeline(
            "blur_v", &bloom_mod, "vs_fullscreen", "fs_blur_v", &blur_bgl, BLOOM_FORMAT,
        );
        let tonemap_pipeline = mk_pipeline(
            "tonemap", &tonemap_mod, "vs_fullscreen", "fs_tonemap", &tonemap_bgl, swapchain_format,
        );

        let bloom = BloomTextures::new(device, width, height);

        let (pf_bg, blur_h_bg, blur_v_bg, tonemap_bg) = Self::make_bind_groups(
            device, &pf_bgl, &blur_bgl, &tonemap_bgl,
            hdr_view, &bloom, &sampler_linear,
            &bloom_params_buf, &tonemap_params_buf,
        );
    ...
    fn make_bind_groups(
        ...
        let pf_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry { binding: 2, resource: bloom_params_buf.as_entire_binding() },
            ],
        });
        let blur_h_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            layout:  blur_bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&bloom.view_a) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(sampler) },
            ],
        });
        let blur_v_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            layout:  blur_bgl,
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&bloom.view_b) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(sampler) },
            ],
        });
        let tonemap_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(hdr_view) },
                wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(&bloom.view_a) },
                wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::Sampler(sampler) },
                wgpu::BindGroupEntry { binding: 3, resource: tonemap_params_buf.as_entire_binding() },
            ],
        });
    ...
    pub fn resize(
        ...
        let (pf_bg, blur_h_bg, blur_v_bg, tonemap_bg) = Self::make_bind_groups(
            device, &self.pf_bgl, &self.blur_bgl, &self.tonemap_bgl,
            hdr_view, &self.bloom, &self.sampler_linear,
            &self.bloom_params_buf, &self.tonemap_params_buf,
        );
        self.pf_bg      = pf_bg;
