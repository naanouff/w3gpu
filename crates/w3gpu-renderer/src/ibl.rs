...
const PREFILTERED_SAMPLES: u32 = 256;
...
impl IblContext {
    ...
    pub fn from_hdr(hdr: &HdrImage, device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        ...
        for face in 0..6usize {
            let face_data = compute_irradiance_face(hdr, face, IRRADIANCE_SIZE, IRRADIANCE_SAMPLES);
            upload_cubemap_layer(queue, &irr_tex, face as u32, 0, IRRADIANCE_SIZE, IRRADIANCE_SIZE, &face_data);
        }

        log::info!("IBL: computing prefiltered env map ({}×{}×6×{} mips)…", PREFILTERED_SIZE, PREFILTERED_SIZE, MAX_MIP_LEVELS);
        let pre_tex = create_cubemap(device, PREFILTERED_SIZE, PREFILTERED_SIZE, MAX_MIP_LEVELS);
        for mip in 0..MAX_MIP_LEVELS {
            let roughness = mip as f32 / (MAX_MIP_LEVELS - 1) as f32;
            let mip_size = (PREFILTERED_SIZE >> mip).max(1);
            for face in 0..6usize {
                let face_data = compute_prefiltered_face(hdr, face, mip_size, roughness, PREFILTERED_SAMPLES);
                upload_cubemap_layer(queue, &pre_tex, face as u32, mip, mip_size, mip_size, &face_data);
            }
        }

        log::info!("IBL: computing BRDF LUT ({}×{})…", BRDF_LUT_SIZE, BRDF_LUT_SIZE);
        let brdf_data = compute_brdf_lut(BRDF_LUT_SIZE, BRDF_LUT_SAMPLES);
        let brdf_tex = create_brdf_lut_texture(device, BRDF_LUT_SIZE);
        upload_brdf_lut(queue, &brdf_tex, &brdf_data, BRDF_LUT_SIZE);
...
fn create_cubemap(device: &wgpu::Device, width: u32, height: u32, mip_levels: u32) -> wgpu::Texture {
...
fn build_context(
    ...
    let prefiltered_view = pre_tex.create_view(&wgpu::TextureViewDescriptor {
        dimension: Some(wgpu::TextureViewDimension::Cube),
        array_layer_count: Some(6),
        mip_level_count: Some(MAX_MIP_LEVELS.min(pre_tex.mip_level_count())),
        ..Default::default()
    });
    ...
    let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
        ...
        mag_filter: if irr_size > 1 { wgpu::FilterMode::Linear } else { wgpu::FilterMode::Nearest },
...
fn compute_irradiance_face(hdr: &HdrImage, face: usize, size: u32, samples: u32) -> Vec<u8> {
    ...
    for py in 0..size {
        for px in 0..size {
            ...
            push_rgba16f(&mut data, acc[0] * inv, acc[1] * inv, acc[2] * inv, 1.0);
        }
    }
    data
}

fn compute_prefiltered_face(hdr: &HdrImage, face: usize, size: u32, roughness: f32, samples: u32) -> Vec<u8> {
    let mut data = Vec::with_capacity((size * size * 8) as usize);
    for py in 0..size {
        for px in 0..size {
            let n = face_direction(face, px, py, size);
            ...
            let (r, g, b) = if total_weight > 0.0 {
                (total_color[0] / total_weight, total_color[1] / total_weight, total_color[2] / total_weight)
            } else {
                (0.0, 0.0, 0.0)
            };
            push_rgba16f(&mut data, r, g, b, 1.0);
        }
    }
    data
}
        ...
        for px in 0..size {
            ...
            for i in 0..samples {
                ...
                if ndotl > 0.0 {
                    ...
                    let g_vis = g * vdoth / (ndoth * ndotv + 1e-4);
            ...
            let inv = 1.0 / samples as f32;
            push_rg16f(&mut data, (a * inv).clamp(0.0, 1.0), (b * inv).clamp(0.0, 1.0));
        }
    }
    data
}
...
fn sample_equirect(hdr: &HdrImage, dir: [f32; 3]) -> [f32; 3] {
    ...
    let px = ((u * hdr.width as f32) as u32).min(hdr.width - 1);
    let py = ((v * hdr.height as f32) as u32).min(hdr.height - 1);
    hdr.pixels[(py * hdr.width + px) as usize]
}
...
#[inline] fn normalize(v: [f32; 3]) -> [f32; 3] {
    ...
    if len < 1e-7 { [0.0, 1.0, 0.0] } else { [v[0]/len, v[1]/len, v[2]/len] }
}
...
fn f32_to_f16(x: f32) -> u16 {
    ...
    else { sign | ((exp as u16) << 10) | (mant >> 13) as u16 }
}
...
mod tests {
    ...
    #[test]
    fn f16_zero() { assert_eq!(f32_to_f16(0.0), 0x0000); }

    #[test]
    fn f16_one() { assert_eq!(f32_to_f16(1.0), 0x3C00); }

    #[test]
    fn f16_two() { assert_eq!(f32_to_f16(2.0), 0x4000); }

    #[test]
    fn f16_large_clamps_to_inf() { assert_eq!(f32_to_f16(1e10), 0x7C00); }

    #[test]
    fn f16_negative_one() { assert_eq!(f32_to_f16(-1.0), 0xBC00); }
    ...
    #[test]
    fn geometry_smith_ibl_perfect_alignment() {
        // roughness=0 → k=0 → g1=g2=1 → result=1
        let g = geometry_smith_ibl(1.0, 1.0, 0.0);
        assert!((g - 1.0).abs() < 1e-6);
    }

    #[test]
    fn geometry_smith_ibl_high_roughness_attenuates() {
        let g = geometry_smith_ibl(0.5, 0.5, 1.0);
        assert!(g < 1.0);
        assert!(g > 0.0);
    }
